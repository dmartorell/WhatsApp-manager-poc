...
import {
  RecentMessageSchema,
  RecentMediaMessageSchema,
  PendingEmailQueueSchema,
  MessageForEmailSchema,
  AutoReplyInfoSchema,
  DbCountSchema,
  PragmaColumnSchema,
  type RecentMessage,
  type RecentMediaMessage,
  type PendingEmailQueue,
  type MessageForEmail,
} from './schemas.js';
import { COST_PER_CLASSIFICATION_USD, USD_TO_EUR_RATE } from './constants.js';
...
const rawColumns = db.prepare('PRAGMA table_info(messages)').all();
const columns = rawColumns
  .map((c) => PragmaColumnSchema.safeParse(c))
  .filter((r): r is { success: true; data: { name: string } } => r.success)
  .map((r) => r.data);
...
export function getRecentMessageFromUser(phone: string, seconds: number): RecentMessage | null {
  const stmt = db.prepare(`
    ...
    LIMIT 1
  `);
  const result = stmt.get(phone, seconds);
  const parsed = RecentMessageSchema.safeParse(result);
...
export function getUsageStats(): UsageStats {
  ...
  const totalParsed = DbCountSchema.safeParse(totalRow);
  const totalMessages = totalParsed.success ? totalParsed.data.count : 0;
  ...
  `).get();
  const classParsed = DbCountSchema.safeParse(classRow);
  const classificationsCount = classParsed.success ? classParsed.data.count : 0;

  const estimatedCostUsd = classificationsCount * COST_PER_CLASSIFICATION_USD;
  const estimatedCostEur = estimatedCostUsd * USD_TO_EUR_RATE;
...
export function getRecentMediaWithoutText(phone: string, seconds: number): RecentMediaMessage | null {
  ...
  `);
  const result = stmt.get(phone, seconds);
  const parsed = RecentMediaMessageSchema.safeParse(result);
  return parsed.success ? parsed.data : null;
}
...
export function getPendingEmails(windowSeconds: number): PendingEmailQueue[] {
  ...
  `);
  const results = stmt.all(windowSeconds);
  return results
    .map((r) => PendingEmailQueueSchema.safeParse(r))
    .filter((p): p is { success: true; data: PendingEmailQueue } => p.success)
    .map((p) => p.data);
}
...
export function getUnsentMessagesForUser(fromPhone: string): MessageForEmail[] {
  ...
  `);
  const results = stmt.all(fromPhone);
  return results
    .map((r) => MessageForEmailSchema.safeParse(r))
    .filter((p): p is { success: true; data: MessageForEmail } => p.success)
    .map((p) => p.data);
}
...
export function markMessagesAsEmailed(messageIds: number[]): void {
  ...
  const validIds = messageIds.filter((id) => Number.isFinite(id) && id > 0);
...
export function getAutoReplyInfo(fromPhone: string): AutoReplyInfo | null {
  ...
  const result = stmt.get(fromPhone);
  const parsed = AutoReplyInfoSchema.safeParse(result);
  ...
  return {
    ...
    needsAutoReply: parsed.data.any_reply_sent === 0,
  };
}
...
export function classifyUserMessages(
  ...
  `);
  stmt.run(category, summary, assignedTo, classificationId, ...validIds);
}
